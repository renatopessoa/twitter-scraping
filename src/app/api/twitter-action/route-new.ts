...
async function handleTweetAction(action: string, tweetId: string) {
  ...
  try {
    ...
    // Estratégia simplificada: usar a URL direta do tweet
    ...
    if (!tweetExists) {
      console.log("Tweet não encontrado, tentando estratégia alternativa...");
      
      // Fallback: ir para o home e procurar o tweet
      await page.goto('https://x.com/home', { timeout: 10000 });
      await page.waitForTimeout(2000);
      
      // Fazer scroll para tentar encontrar o tweet
      for (let i = 0; i < 5; i++) {
        const foundTweet = await page.locator(`[href*="/status/${tweetId}"]`).first().isVisible();
        if (foundTweet) {
          await page.locator(`[href*="/status/${tweetId}"]`).first().click();
          await page.waitForTimeout(2000);
          break;
        }
        await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    ...
    if (action === 'like') {
      ...
      await retweetButton.waitFor({ state: 'visible', timeout: 10000 });
